            ...
            try {
                if (err) {
                    ...
                    if (typeof data == 'string') {
                        res = JSON.parse(data);
                        if (res.code == 1000) {
                            $.userSynthesizeScore = res.rs.userSynthesizeScore;
                            $.userDimensionScore = res.rs.userDimensionScore;
                            $.scoreUserInfo = res.rs.scoreUserInfo;

                            $.baiScore = $.userDimensionScore.baiScore;
                            $.totalScore = $.userSynthesizeScore.totalScore;
                            $.level = $.userSynthesizeScore.level;

                            console.log(`综合等级: ${$.level} 综合得分: ${$.totalScore} 维度得分:${$.baiScore}`);

                            if ($.baiScore < 2 || $.level != "R0" || $.totalScore < 90) {
                                console.log(`【${$.index}${$.UserName}】 - 是黑号吧`);
                            } else if ($.level == "ST" || $.baiScore < 3) {
                                console.log(`【${$.index}${$.UserName}】 - 有点危险`);
                            } else {
                                console.log(`安全账号`);
                            }
