...
[task_local]
#5G超级盲盒
5 0,1-23/3 * * * https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_mohe.js, tag=5G超级盲盒, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true

=================================Loon===================================
[Script]
cron "5 0,1-23/3 * * *" script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_mohe.js,tag=5G超级盲盒

===================================Surge================================
5G超级盲盒 = type=cron,cronexp="0 0,1-23/3 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_mohe.js

====================================小火箭=============================
5G超级盲盒 = type=cron,script-path=https://raw.githubusercontent.com/he1pu/JDHelp/main/jd_mohe.js, cronexpr="0 0,1-23/3 * * *", timeout=3600, enable=true
...
function submitCode(myInviteCode, user) {
    return new Promise(async resolve => {
    $.get({url: `https://www.appinstall.win/jdcodes/submit.php?code=${myInviteCode}&type=mohe&user=${user}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({
      url: `https://www.appinstall.win/jdcodes/getcode.php?type=mohe&num=20`,
      'timeout': 10000
    }, (err, resp, data) => {
      try {
        if (err) {
